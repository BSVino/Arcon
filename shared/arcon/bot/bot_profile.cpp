...
#include "bot.h"
#include "bot_util.h"
#include "cf_bot.h"		// BOTPORT: Remove this CF dependency
...
const char* BotProfile::GetWeaponPreferenceAsString( int i ) const
{
	...
	return WeaponIDToAlias( (CFWeaponID)m_weaponPreference[ i ] );
}
	...
	while( true )
	{
		...
		while( true )
		{
			...
			else if (!stricmp( "Team", attributeName ))
			{
				if ( !stricmp( token, "N" ) )
				{
					profile->m_teams = TEAM_NUMENI;
				}
				else if ( !stricmp( token, "M" ) )
				{
					profile->m_teams = TEAM_MACHINDO;
				}
				else
				{
 ...
 */
const BotProfile *BotProfileManager::GetRandomProfile( BotDifficultyType difficulty, int team, CFWeaponType weaponType ) const
{
	...
	FOR_EACH_LL( m_profileList, it )
	{
		...
		if ( weaponType != WT_NONE )
		{
			...
			if ( weaponType != WeaponClassFromWeaponID( (CFWeaponID)profile->GetWeaponPreference( 0 ) ) )
				continue;
		}

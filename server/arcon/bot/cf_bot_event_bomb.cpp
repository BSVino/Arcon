...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnBombPickedUp( IGameEvent *event )
{
	if ( !IsAlive() )
		return;
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnBombPlanted( IGameEvent *event )
{
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnBombBeep( IGameEvent *event )
{
	if ( !IsAlive() )
		return;
...
/*			if (GetTeamNumber() == TEAM_CT && GetGameState()->GetPlantedBombsite() == CFGameState::UNKNOWN)
			{
				const CCFBotManager::Zone *zone = TheCFBots()->GetZone( entity->GetAbsOrigin() );
				if (zone)
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnBombDefuseBegin( IGameEvent *event )
{
}


//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnBombDefused( IGameEvent *event )
{
...
/*	if (GetTeamNumber() == TEAM_CT)
	{
		if (TheCFBots()->GetBombTimeLeft() < 2.0f)
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnBombDefuseAbort( IGameEvent *event )
{
	if ( !IsAlive() )
		return;
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnBombExploded( IGameEvent *event )
{

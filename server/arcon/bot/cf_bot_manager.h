...
inline CCFBotManager *TheCFBots( void )
{
	return reinterpret_cast< CCFBotManager * >( TheBots );
}
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Macro to set up an OnEventClass() in TheCFBots.
 */
...
//--------------------------------------------------------------------------------------------------------------
#define DECLARE_CFBOTMANAGER_EVENT_LISTENER( EventClass, EventName ) DECLARE_BOTMANAGER_EVENT_LISTENER( TheCFBots, EventClass, EventName )
 ...
 */
#define CCFBOTMANAGER_ITERATE_BOTS( Callback, arg1 ) \
	{ \
...
//
class CCFBotManager : public CBotManager
{
public:
	CCFBotManager();
	...
	virtual bool IsImportantPlayer( CCFPlayer *player ) const;				///< return true if player is important to scenario (VIP, bomb carrier, etc)
	...
	CCFPlayer *GetBombDefuser( void ) const		{ return m_bombDefuser; }			///< return the player currently defusing the bomb, or NULL
	...
	bool AllowLongSwords( void ) const				{ return cv_bot_allow_longswords.GetBool(); }
	bool AllowLongGuns( void ) const				{ return cv_bot_allow_longguns.GetBool(); }
	bool AllowBusters( void ) const					{ return cv_bot_allow_busters.GetBool(); }

	bool AllowFriendlyFireDamage( void ) const		{ return friendlyfire.GetBool(); }

	bool IsWeaponUseable( const CWeaponCFBase *weapon ) const;	///< return true if the bot can use this weapon

	bool IsDefenseRushing( void ) const				{ return m_isDefenseRushing; }		///< returns true if defense team has "decided" to rush this round
	bool IsOnDefense( const CCFPlayer *player ) const;		///< return true if this player is on "defense"
	bool IsOnOffense( const CCFPlayer *player ) const;		///< return true if this player is on "offense"

	bool IsRoundOver( void ) const					{ return m_isRoundOver; }		///< return true if the round has ended

	#define FROM_CONSOLE true
	bool BotAddCommand( int team, bool isFromConsole = false, const char *profileName = NULL, CFWeaponType weaponType = WT_NONE, BotDifficultyType difficulty = NUM_DIFFICULTY_LEVELS );	///< process the "bot_add" console command
...
private:
	...
	CCFPlayer *m_bombDefuser;								///< the player currently defusing a bomb
	...
	DECLARE_CFBOTMANAGER_EVENT_LISTENER( BombPickedUp,			bomb_pickup )
	DECLARE_CFBOTMANAGER_EVENT_LISTENER( BombPlanted,			bomb_planted )
	DECLARE_CFBOTMANAGER_EVENT_LISTENER( BombBeep,				bomb_beep )
	DECLARE_CFBOTMANAGER_EVENT_LISTENER( BombDefuseBegin,		bomb_begindefuse )
	DECLARE_CFBOTMANAGER_EVENT_LISTENER( BombDefused,			bomb_defused )
	DECLARE_CFBOTMANAGER_EVENT_LISTENER( BombDefuseAbort,		bomb_abortdefuse )
	DECLARE_CFBOTMANAGER_EVENT_LISTENER( BombExploded,			bomb_exploded )
...
inline CBasePlayer *CCFBotManager::AllocateBotEntity( void )
{
	return static_cast<CBasePlayer *>( CreateEntityByName( "cf_bot" ) );
}

inline bool CCFBotManager::IsTimeToPlantBomb( void ) const
{
	return (gpGlobals->curtime >= m_earliestBombPlantTimestamp);
}

inline const CCFBotManager::Zone *CCFBotManager::GetClosestZone( const CBaseEntity *entity ) const
{
	if (entity == NULL)
		return NULL;

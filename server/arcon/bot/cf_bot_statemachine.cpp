 ...
 */
void CCFBot::SetState( BotState *state )
{
	...
	if ( IsDefusingBomb() )
	{
		...
		if ( bombPos != NULL )
		{
			if ( TheCFBots()->GetBombDefuser() == this )
			{
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::EscapeFromBomb( void )
{
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::Follow( CCFPlayer *player )
{
	if (player == NULL)
		return;
 ...
 */
void CCFBot::ContinueFollowing( void )
{
	SetTask( FOLLOW );
 ...
 * Stop following
 */
void CCFBot::StopFollowing( void )
{
	m_isFollowing = false;
	m_leader = NULL;
 ...
 * Use the entity
 */
void CCFBot::UseEntity( CBaseEntity *entity )
{
 ...
 */
void CCFBot::OpenDoor( CBaseEntity *door )
{
 ...
 */
void CCFBot::Hide( CNavArea *searchFromArea, float duration, float hideRange, bool holdPosition )
{
	DestroyPath();
 ...
 */
void CCFBot::Hide( const Vector &hidingSpot, float duration, bool holdPosition )
{
 ...
 * If 'searchFromArea' is non-NULL, hiding spots are looked for from that area first.
 */
bool CCFBot::TryToHide( CNavArea *searchFromArea, float duration, float hideRange, bool holdPosition, bool useNearest )
{
	CNavArea *source;
	Vector sourcePos;
	if (searchFromArea)
 ...
 */
bool CCFBot::TryToRetreat( float maxRange, float duration )
{
 ...
 */
void CCFBot::Attack( CCFPlayer *victim )
{
	if (victim == NULL)
		return;
 ...
 */
void CCFBot::StopAttacking( void )
{
...
//--------------------------------------------------------------------------------------------------------------
bool CCFBot::IsAttacking( void ) const
{
 ...
 */
bool CCFBot::IsEscapingFromBomb( void ) const
{
 ...
 */
bool CCFBot::IsDefusingBomb( void ) const
{
 ...
 */
bool CCFBot::IsHiding( void ) const
{
 ...
 */
bool CCFBot::IsAtHidingSpot( void ) const
{
	if (!IsHiding())
		return false;
 ...
 */
float CCFBot::GetHidingTime( void ) const
{
	if (IsHiding())
	{
 ...
 */
bool CCFBot::IsHunting( void ) const
{
 ...
 */
bool CCFBot::IsMovingTo( void ) const
{
 ...
 */
bool CCFBot::IsBuying( void ) const
{
...
//--------------------------------------------------------------------------------------------------------------
bool CCFBot::IsInvestigatingNoise( void ) const
{
 ...
 */
void CCFBot::MoveTo( const Vector &pos, RouteType route )
{
 ...
 */
void CCFBot::InvestigateNoise( void )
{
 ...
 */
bool CCFBot::MoveToInitialEncounter( void )
{
	int myTeam = GetTeamNumber();
	int enemyTeam = OtherTeam( myTeam );

	// build a path to an enemy spawn point
	CBaseEntity *enemySpawn = TheCFBots()->GetRandomSpawn( enemyTeam );

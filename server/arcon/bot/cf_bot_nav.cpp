 ...
 */
CBaseEntity * CCFBot::FindEntitiesOnPath( float distance, CPushAwayEnumerator *enumerator, bool checkStuck )
{
	Vector goal;
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::PushawayTouch( CBaseEntity *pOther )
{
#if DEBUG_BREAKABLES
 ...
 * stopped by it.
 */
void CCFBot::BreakablesCheck( void )
{
	if ( !HasPath() )
		return;
 ...
 */
void CCFBot::ResetStuckMonitor( void )
{
	if (m_isStuck)
	{
 ...
 */
bool CCFBot::DiscontinuityJump( float ground, bool onlyJumpDown, bool mustJump )
{
 ...
 */
bool CCFBot::GetSimpleGroundHeightWithFloor( const Vector &pos, float *height, Vector *normal )
{
 ...
 */
Place CCFBot::GetPlace( void ) const
{
	if (m_lastKnownArea)
 ...
 */
void CCFBot::MoveTowardsPosition( const Vector &pos )
{
 ...
 */
void CCFBot::MoveAwayFromPosition( const Vector &pos )
{
	// compute our current forward and lateral vectors
	float angle = EyeAngles().y;
 ...
 */
void CCFBot::StrafeAwayFromPosition( const Vector &pos )
{
	// compute our current forward and lateral vectors
	float angle = EyeAngles().y;
 ...
 */
void CCFBot::ComputeApproachPoints( void )
{
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::DrawApproachPoints( void ) const
{
	for( int i=0; i<m_approachPointCount; ++i )
	{
		if (TheCFBots()->GetElapsedRoundTime() >= m_approachPoint[i].m_area->GetEarliestOccupyTime( OtherTeam( GetTeamNumber() ) ))
 ...
 */
bool CCFBot::FindApproachPointNearestPath( Vector *pos )
{
	if (!HasPath())
		return false;
 ...
 */
bool CCFBot::IsAtEnemySpawn( void ) const
{
	CBaseEntity *spot;

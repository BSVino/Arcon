 ...
 */
void HideState::OnEnter( CCFBot *me )
{
	m_isAtSpot = false;
 ...
 * NOTE: Do not forget this state may include a very long "move-to" time to get to our hidey spot!
 */
void HideState::OnUpdate( CCFBot *me )
{
	...
	if (!me->IsReloading())
	{
		...
		if (me->IsFollowing())
		{
			CCFPlayer *leader = static_cast<CCFPlayer *>( static_cast<CBaseEntity *>( me->GetFollowLeader() ) );
		...
		if (!me->IsReloading() && 
			!isSettledInSniper && 
			me->GetDisposition() == CCFBot::ENGAGE_AND_INVESTIGATE)
		{
	...
	if (m_isAtSpot)
	{
		...
		{
			if (me->GetTask() == CCFBot::GUARD_LOOSE_BOMB)
			{
				// if we're guarding the loose bomb, continue to guard it but pick a new spot
				me->Hide( TheCFBots()->GetLooseBombArea() );
				return;
			}
			else if (me->GetTask() == CCFBot::GUARD_BOMB_ZONE)
			{
				...
				if (zone)
				{
					CNavArea *area = TheCFBots()->GetRandomAreaInZone( zone );
					if (area)
					{
						me->Hide( area );
						return;
...
*/
	...
	else
	{
		...
		float range;
		CCFPlayer *camper = static_cast<CCFPlayer *>( UTIL_GetClosestPlayer( m_hidingSpot, &range ) );
		...
		if (range < atDist)
		{
			...
			if (me->GetTask() == CCFBot::MOVE_TO_SNIPER_SPOT)
			{
				me->SetTask( CCFBot::SNIPING );
			}
			else if (me->GetTask() == CCFBot::GUARD_INITIAL_ENCOUNTER)
			{
		...
		// move to hiding spot
		if (me->UpdatePathMovement() != CCFBot::PROGRESSING && !m_isAtSpot)
		{
...
//--------------------------------------------------------------------------------------------------------------
void HideState::OnExit( CCFBot *me )
{

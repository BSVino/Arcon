...
//-----------------------------------------------------------------------------------------------------------
float CCFBot::GetMoveSpeed( void )
{
	return 250.0f;
}
 ...
 */
void CCFBot::Upkeep( void )
{
	VPROF_BUDGET( "CCFBot::Upkeep", VPROF_BUDGETGROUP_NPCS );
 ...
 */
void CCFBot::Update( void )
{
	VPROF_BUDGET( "CCFBot::Update", VPROF_BUDGETGROUP_NPCS );
	...
	CCFPlayer *threat = GetRecognizedEnemy();
	if (threat)
	{
		...
		TheCFBots()->SetLastSeenEnemyTimestamp();
	}
	...
	{
		StrafeAwayFromPosition( ToCFPlayer(m_avoid)->GetCentroid() );
	}
	else
	{
		m_avoid = NULL;
...

	{
		...
		if (CanSeeLooseBomb())
			GetChatter()->SpottedLooseBomb( TheCFBots()->GetLooseBomb() );
	}
	...
	if (cv_bot_auto_follow.GetBool() &&
		TheCFBots()->GetElapsedRoundTime() > earliestAutoFollowTime &&
		...
		!GetGameState()->IsAtPlantedBombsite())
	{
		...
		{
			CCFPlayer *leader = GetClosestVisibleHumanFriend();
			if (leader)
			{
...
{
public:
	DrawTravelTime( const CCFBot *me )
	{
		m_me = me;
	}
	...
	bool operator() ( CBasePlayer *player )
	{
		if (player->IsAlive() && !m_me->InSameTeam( player ))
		{
			CFmtStr msg;
			player->EntityText(	0,
								msg.sprintf( "%3.0f", m_me->GetTravelDistanceToPlayer( (CCFPlayer *)player ) ),
								0.1f );


			if (m_me->DidPlayerJustFireWeapon( ToCFPlayer( player ) ))
			{
 ...
 */
void CCFBot::DebugDisplay( void ) const
{
	...
	else
	{
		...
		if (false && m_spotEncounter)
		{
			...
			FOR_EACH_LL( m_spotEncounter->spotList, it )
			{
				...
				{
					if (TheCFBots()->GetElapsedRoundTime() < order->spot->GetArea()->GetEarliestOccupyTime( OtherTeam( GetTeamNumber() ) ))
					{
						continue;
					}
				}
	...
	if (false)
	{
		CStudioHdr *pStudioHdr = const_cast< CCFBot *>( this )->GetModelPtr();
		if ( !pStudioHdr )
			return;

		mstudiohitboxset_t *set = pStudioHdr->pHitboxSet( const_cast< CCFBot *>( this )->GetHitboxSet() );
		if ( !set )
			return;
		...
		for ( int i = 0; i < set->numhitboxes; i++ )
		{
			...
			const_cast< CCFBot *>( this )->GetBonePosition( pbox->bone, position, angles );
	...
	/*
	const QAngle &angles = const_cast< CCFBot *>( this )->GetPunchAngle();
 ...
 */
void CCFBot::UpdateTravelDistanceToAllPlayers( void )
{

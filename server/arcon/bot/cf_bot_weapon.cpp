 ...
 */
void CCFBot::FireWeaponAtEnemy( void )
{
		...
		!IsSurprised())
	{
			...
			IsEnemyVisible())
		{
			...
			if (onTarget > aimTolerance)
			{
				...
				if (TheCFBots()->AllowFriendlyFireDamage())
				{
				...
				if (doAttack)
				{
					...
					if (IsUsingPrimaryMelee())
					{
						...
						if (rangeToEnemy < meleeRange)
						{
							...
							CCFPlayer* pCFEnemy = ToCFPlayer(enemy);
							bool bEnemyUsingMelee = (pCFEnemy->GetPrimaryWeapon() && pCFEnemy->GetPrimaryWeapon()->IsMeleeWeapon());
							...
							{
								if (RandomInt(0, 1) == 0)	// All melee weapons are semi, so spam it.
									SecondaryAttack();
							}
							else
							{
								...
								if (GetEnemyFrozenUntil() > gpGlobals->curtime + GetPrimaryWeapon()->GetFireRate()/2)
								{
									if (RandomInt(0, 1) == 0)	// All melee weapons are semi, so spam it.
										SecondaryAttack();
								}
						...
						else if (HasBackupFirearm())
						{
							if (GetPrimaryWeapon()->IsFullAuto())
								SecondaryAttack();
					...
					else
					{
						...
						if (GetSecondaryWeapon() && !GetSecondaryWeapon()->IsMeleeWeapon() && !GetSecondaryWeapon()->IsDisableFlagged())
						{
							if (GetSecondaryWeapon()->IsFullAuto())
								SecondaryAttack();
							else if (RandomInt(0, 1) == 0)
								SecondaryAttack();
						}
 ...
 */
void CCFBot::SetAimOffset( float accuracy )
{
 ...
 */
void CCFBot::UpdateAimOffset( void )
{
 ...
 */
bool CCFBot::AdjustZoom( float range )
{
 ...
 */
bool CCFBot::IsUsing( CFWeaponID weaponID ) const
{
	CWeaponCFBase *weapon = GetActiveCFWeapon();
 ...
 */
bool CCFBot::IsUsingSniperRifle( void ) const
{
	return false;
}
 ...
 */
bool CCFBot::IsSniper( void ) const
{
	return false;
}
 ...
 */
bool CCFBot::IsSniping( void ) const
{
 ...
 */
bool CCFBot::IsUsingShotgun( void ) const
{
	CWeaponCFBase *weapon = GetPrimaryWeapon();
 ...
 */
bool CCFBot::IsUsingMachinegun( void ) const
{
	return false;
}
 ...
 */
bool CCFBot::IsPrimaryWeaponEmpty( void ) const
{
	if (GetPrimaryWeapon())
		return GetPrimaryWeapon()->HasAnyAmmo();
 ...
 */
bool CCFBot::IsSecondaryWeaponEmpty( void ) const
{
	if (GetSecondaryWeapon())
		return GetSecondaryWeapon()->HasAnyAmmo();
 ...
 */
bool CCFBot::DoEquip( CWeaponCFBase *weapon )
{
 ...
 */
void CCFBot::EquipBestWeapon( bool mustEquip )
{
	// throttle how often equipping is allowed
	...
	CWeaponCFBase* pInactive = GetInactiveSecondaryWeapon();

	if (pInactive && !pSecondary || pSecondary && !pInactive)
	{
		DoEquip(pInactive);
		return;
	}

	if (!pInactive && !pSecondary)
		return;

	if (!pSecondary->HasAnyAmmo() && pInactive->HasAnyAmmo())
	{
		DoEquip(pInactive);
		return;
	}
 ...
 * Equip our pistol
 */
void CCFBot::EquipSecondary( void )
{
	// throttle how often equipping is allowed
 ...
 */
bool CCFBot::HasGrenade( void ) const
{
	return false;
}
 ...
 */
bool CCFBot::EquipGrenade( bool noSmoke )
{
...
bool CCFBot::IsUsingPrimaryMelee( void ) const
{
	return GetPrimaryWeapon() && GetPrimaryWeapon()->IsMeleeWeapon();
}

bool CCFBot::HasBackupFirearm( void ) const
{
	return GetSecondaryWeapon() && !GetSecondaryWeapon()->IsMeleeWeapon();
}
 ...
 */
bool CCFBot::IsUsingGrenade( void ) const
{
	return false;
}
 ...
 */
void CCFBot::ThrowGrenade( const Vector &target )
{
 ...
 */
void CCFBot::LookForGrenadeTargets( void )
{
...

	{
		...
		if (enemyTime - TheCFBots()->GetElapsedRoundTime() > leadTime)
		{
			// don't throw yet
			return;
		}
 ...
 */
void CCFBot::UpdateGrenadeThrow( void )
{
 ...
 */
void CCFBot::AvoidEnemyGrenades( void )
{
...
void CCFBot::ReloadCheck( void )
{
	...
	if (IsActiveWeaponClipEmpty())
	{
		...
		{
			if (GetSecondaryWeapon() && !GetSecondaryWeapon()->IsMeleeWeapon() && !IsSecondaryWeaponEmpty())
			{
 ...
 */
bool CCFBot::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
 ...
 */
bool CCFBot::IsFriendInLineOfFire( void )
{
 ...
 */
float CCFBot::ComputeWeaponSightRange( void )
{
	// compute the unit vector along our view
	Vector aimDir = GetViewVector();
 ...
 */
bool CCFBot::DidPlayerJustFireWeapon( const CCFPlayer *player ) const
{
	// if this player has just fired his weapon, we notice him
	CWeaponCFBase *weapon = player->GetPrimaryWeapon();

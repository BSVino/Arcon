 ...
 */
bool CCFBot::ComputePathPositions( void )
{
	if (m_pathLength == 0)
		return false;
 ...
 */
void CCFBot::SetupLadderMovement( void )
{
...
void CCFBot::ComputeLadderEndpoint( bool isAscending )
{
	trace_t result;
	Vector from, to;
 ...
 */
bool CCFBot::UpdateLadderMovement( void )
{
 ...
 */
bool CCFBot::FindClosestPointOnPath( const Vector &worldPos, int startIndex, int endIndex, Vector *close ) const
{
 ...
 */
int CCFBot::FindOurPositionOnPath( Vector *close, bool local ) const
{
	if (!HasPath())
		return -1;
 ...
 */
bool CCFBot::IsStraightLinePathWalkable( const Vector &goal ) const
{
 ...
 */
int CCFBot::FindPathPoint( float aheadRange, Vector *point, int *prevIndex )
{
	Vector myOrigin = GetCentroid();
 ...
 */
void CCFBot::SetPathIndex( int newIndex )
{
 ...
 */
bool CCFBot::IsNearJump( void ) const
{
 ...
 */
float CCFBot::GetApproximateFallDamage( float height ) const
{
 ...
 */
bool CCFBot::IsFriendInTheWay( const Vector &goalPos )
{
	...
	{
		CCFPlayer *player = static_cast<CCFPlayer *>( UTIL_PlayerByIndex( i ) );
 ...
 */
void CCFBot::FeelerReflexAdjustment( Vector *goalPosition )
{
 ...
 */
bool CCFBot::IsRunning( void ) const
{
 ...
 */
CCFBot::PathResult CCFBot::UpdatePathMovement( bool allowSpeedChange )
{
	VPROF_BUDGET( "CCFBot::UpdatePathMovement", VPROF_BUDGETGROUP_NPCS );
 ...
 */
void CCFBot::BuildTrivialPath( const Vector &goal )
{
	Vector myOrigin = GetCentroid();
 ...
 */
bool CCFBot::ComputePath( const Vector &goal, RouteType route )
{
	VPROF_BUDGET( "CCFBot::ComputePath", VPROF_BUDGETGROUP_NPCS );
 ...
 */
float CCFBot::GetPathDistanceRemaining( void ) const
{
	if (!HasPath())
		return -1.0f;

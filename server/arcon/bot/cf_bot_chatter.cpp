 ...
 */
void BotMeme::Transmit( CCFBot *sender ) const
{
 ...
 */
void BotHelpMeme::Interpret( CCFBot *sender, CCFBot *receiver ) const
{
 ...
 */
void BotBombsiteStatusMeme::Interpret( CCFBot *sender, CCFBot *receiver ) const
{
	...
	if (receiver->GetTask() == CCFBot::FIND_TICKING_BOMB)
	{
		receiver->Idle();
 ...
 */
void BotBombStatusMeme::Interpret( CCFBot *sender, CCFBot *receiver ) const
{
	// update our gamestate based on teammate's report
	switch( m_state )
	{
		case CFGameState::MOVING:
		...
		case CFGameState::LOOSE:
			receiver->GetGameState()->UpdateLooseBomb( m_pos );

			if (receiver->GetTask() == CCFBot::GUARD_BOMB_ZONE)
			{
				receiver->Idle();
				receiver->GetChatter()->Affirmative();		
			}
 ...
 */
void BotFollowMeme::Interpret( CCFBot *sender, CCFBot *receiver ) const
{
	if (receiver->IsRogue())
		return;
 ...
 */
void BotDefendHereMeme::Interpret( CCFBot *sender, CCFBot *receiver ) const
{
	if (receiver->IsRogue())
		return;
	...
	if (place != UNDEFINED_PLACE)
	{
		...
		if (spot)
		{
			receiver->SetTask( CCFBot::HOLD_POSITION );
			receiver->Hide( *spot );
			return;
		}
	}
 ...
 */
void BotWhereBombMeme::Interpret( CCFBot *sender, CCFBot *receiver ) const
{
	int zone = receiver->GetGameState()->GetPlantedBombsite();

	if (zone != CFGameState::UNKNOWN)
 ...
 */
void BotRequestReportMeme::Interpret( CCFBot *sender, CCFBot *receiver ) const
{
 ...
 */
void BotHeardNoiseMeme::Interpret( CCFBot *sender, CCFBot *receiver ) const
{
 ...
 */
void BotWarnSniperMeme::Interpret( CCFBot *sender, CCFBot *receiver ) const
{
...
//---------------------------------------------------------------------------------------------------------------
CCFBot *BotStatement::GetOwner( void ) const
{ 
...
BotChatterInterface::BotChatterInterface( CCFBot *me )
{
	m_me = me;
...
//---------------------------------------------------------------------------------------------------------------
inline const BotPhrase *GetPlacePhrase( CCFBot *me )
{
...
void BotChatterInterface::ReportingIn( void )
{
	...
	switch( m_me->GetTask() )
	{
		...
		case CCFBot::GUARD_LOOSE_BOMB:
		{
			if (TheCFBots()->GetLooseBomb())
			{
				say->AppendPhrase( TheBotPhrases->GetPhrase( "GuardingLooseBomb" ) );
				say->AttachMeme( new BotBombStatusMeme( CFGameState::LOOSE, TheCFBots()->GetLooseBomb()->GetAbsOrigin() ) );
			}
			break;
		}

		case CCFBot::GUARD_VIP_ESCAPE_ZONE:
		{
			break;
		}
...
void BotChatterInterface::TheyPickedUpTheBomb( void )
{
	...
	say->AttachMeme( new BotBombStatusMeme( CFGameState::MOVING, myOrigin ) ); 
...
void BotChatterInterface::SpottedBomber( CBasePlayer *bomber )
{
	...
	//say->AttachMeme( new BotHelpMeme( place ) );
	say->AttachMeme( new BotBombStatusMeme( CFGameState::MOVING, bomberOrigin ) ); 
...
void BotChatterInterface::SpottedLooseBomb( CBaseEntity *bomb )
{
	...
	if (m_spottedLooseBombTimer.IsElapsed())
	{
		...
		if (TheCFBots()->GetLooseBomb())
			say->AttachMeme( new BotBombStatusMeme( CFGameState::LOOSE, bomb->GetAbsOrigin() ) );
...
{
	const CCFBotManager::Zone *zone = TheCFBots()->GetZone( zoneIndex );
	if (zone == NULL)
		return;
...
void BotChatterInterface::CelebrateWin( void )
{
	...
	if (m_me->GetFriendsRemaining() == 0)
	{
		...
		if (TheCFBots()->GetElapsedRoundTime() < quickRound)
...
void BotChatterInterface::AnnouncePlan( const char *phraseName, Place place )
{
	...
	say->SetStartTime( TheCFBots()->GetRoundStartTime() + RandomFloat( 2.0, 3.0f ) );

...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnPlayerDeath( IGameEvent *event )
{
	if ( !IsAlive() )
		return;
	...
	Vector playerOrigin = (player) ? ToCFPlayer(player)->GetCentroid() : Vector( 0, 0, 0 );
	...
	if (victim->GetTeamNumber() == GetTeamNumber())
	{
		...
		else	// not attacking
		{
			...
			if (GetDisposition() == ENGAGE_AND_INVESTIGATE || GetDisposition() == OPPORTUNITY_FIRE)
			{
				...
				{
					CCFPlayer *killer = static_cast<CCFPlayer *>( other );
	...
	else  // an enemy was killed
	{
		...
		if (killer && killer->GetTeamNumber() == GetTeamNumber())
		{
			...
			if (GetNearbyEnemyCount() <= 1)
			{
				...
				Vector victimOrigin = ToCFPlayer(victim)->GetCentroid();
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnPlayerRadio( IGameEvent *event )
{
}


//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnPlayerFallDamage( IGameEvent *event )
{
	if ( !IsAlive() )
		return;
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::OnPlayerFootstep( IGameEvent *event )
{
	if ( !IsAlive() )
		return;

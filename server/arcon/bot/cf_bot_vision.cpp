 ...
 */
inline float StayOnLadderLine( CCFBot *me, const CNavLadder *ladder )
{
...
//--------------------------------------------------------------------------------------------------------------
void CCFBot::ComputeLadderAngles( float *yaw, float *pitch )
{
	if ( !yaw || !pitch )
		return;
 ...
 */
void CCFBot::UpdateLookAngles( void )
{
	VPROF_BUDGET( "CCFBot::UpdateLookAngles", VPROF_BUDGETGROUP_NPCS );
 ...
 */
void CCFBot::SetLookAt( const char *desc, const Vector &pos, PriorityType pri, float duration, bool clearIfClose, float angleTolerance, bool attack )
{
 ...
 */
void CCFBot::InhibitLookAround( float duration )
{
 ...
 */
void CCFBot::UpdatePeripheralVision()
{
	VPROF_BUDGET( "CCFBot::UpdatePeripheralVision", VPROF_BUDGETGROUP_NPCS );
 ...
 */
void CCFBot::UpdateLookAround( bool updateNow )
{
	VPROF_BUDGET( "CCFBot::UpdateLookAround", VPROF_BUDGETGROUP_NPCS );
	...
	if (IsAtHidingSpot() || IsNotMoving( minStillTime ))
	{
		...
		for( int i=0; i<m_approachPointCount; ++i )
		{
			...
			if (TheCFBots()->GetElapsedRoundTime() >= spotTime)
			{
	...
	if (m_spotEncounter)
	{
		...
		if (!IsSafe() && !IsLookingAtSpot( PRIORITY_LOW ))
		{
			...
			FOR_EACH_LL( m_spotEncounter->spotList, it )
			{
				...
				{
					if (TheCFBots()->GetElapsedRoundTime() < spotOrder->spot->GetArea()->GetEarliestOccupyTime( OtherTeam( GetTeamNumber() ) ))
					{
						continue;
					}
				}
 ...
 */
bool CCFBot::BendLineOfSight( const Vector &eye, const Vector &target, Vector *bend, float angleLimit ) const
{
	VPROF_BUDGET( "CCFBot::BendLineOfSight", VPROF_BUDGETGROUP_NPCS );
 ...
 */
bool CCFBot::IsNoticable( const CCFPlayer *player, unsigned char visParts ) const
{
 ...
 */
CCFPlayer *CCFBot::FindMostDangerousThreat( void )
{
	VPROF_BUDGET( "CCFBot::FindMostDangerousThreat", VPROF_BUDGETGROUP_NPCS );
	...
	CCFPlayer *currentThreat = m_enemyQueue[ prevIndex ].player;
	...
	float closeKORange = 99999999999.9f;

	CCFPlayer *sniperThreat = NULL;
	...
	{
		VPROF_BUDGET( "CCFBot::Collect Threats", VPROF_BUDGETGROUP_NPCS );
		...
		for( i = 1; i <= gpGlobals->maxClients; ++i )
		{
			...
			{
				if (IsVisible(entity->WorldSpaceCenter(), true, this))
				{
					// keep track of our closest friend 
					Vector to = GetAbsOrigin() - player->GetAbsOrigin();
					float rangeSq = to.LengthSqr();
					if (rangeSq < closeKORange)
					{
						m_closestVisibleKO = player;
						closeKORange = rangeSq;
					}
				}
				continue;
			}
			...
			if (player->InSameTeam( this ))
			{
				...
				if (IsVisible( entity->WorldSpaceCenter(), false, this ))
				{
					...
					m_watchInfo[idx].isEnemy = (CFGameRules()->PlayerRelationship(this, player) == GR_TEAMMATE);
			...
			if (IsSniperRifle( player->GetPrimaryWeapon() ))
			{
			...
			{
				VPROF_BUDGET( "CCFBot::Sort Threats", VPROF_BUDGETGROUP_NPCS );
	...
	{
		VPROF_BUDGET( "CCFBot::Count nearby Friends & Enemies", VPROF_BUDGETGROUP_NPCS );
	...
	{
		VPROF_BUDGET( "CCFBot::Track enemy Place", VPROF_BUDGETGROUP_NPCS );
		...
		for( i=0; i<threatCount; ++i )
		{
			...
			CNavArea *area;
			CCFBot *bot = dynamic_cast<CCFBot *>(threat[i].enemy);
			if (bot && bot->IsBot())
			{
	...
	{
		VPROF_BUDGET( "CCFBot::Select Threat", VPROF_BUDGETGROUP_NPCS );
 ...
 */
void CCFBot::UpdateReactionQueue( void )
{
	VPROF_BUDGET( "CCFBot::UpdateReactionQueue", VPROF_BUDGETGROUP_NPCS );
	...
	CCFPlayer *threat = FindMostDangerousThreat();
 ...
 */
CCFPlayer *CCFBot::GetRecognizedEnemy( void )
{
 ...
 */
bool CCFBot::IsRecognizedEnemyReloading( void )
{
	if (m_enemyQueueAttendIndex >= m_enemyQueueCount)
		return false;
 ...
 */
bool CCFBot::IsRecognizedEnemyProtectedByShield( void )
{
	if (m_enemyQueueAttendIndex >= m_enemyQueueCount)
		return false;
 ...
 */
float CCFBot::GetRangeToNearestRecognizedEnemy( void )
{
	const CCFPlayer *enemy = GetRecognizedEnemy();
 ...
 */
void CCFBot::Blind( float holdTime, float fadeTime, float startingAlpha )
{
...
{
public:
	CheckLookAt( const CCFBot *me, bool testFOV )
	{
		m_me = me;
		m_testFOV = testFOV;
	}
	...
	bool operator() ( CBasePlayer *player )
	{
		...
		if (m_testFOV && !(const_cast< CCFBot * >(m_me)->FInViewCone( player->WorldSpaceCenter() )))
			return true;

		if (!m_me->IsPlayerLookingAtMe( player ))
			return true;
			
		if (m_me->IsVisible( (CCFPlayer *)player ))
			return false;
 ...
 */
bool CCFBot::IsAnyVisibleEnemyLookingAtMe( bool testFOV ) const
{
 ...
 * Do panic behavior
 */
void CCFBot::UpdatePanicLookAround( void )
{

...
{
public:
	void Transmit( CCFBot *sender ) const;									///< transmit meme to other bots
	virtual void Interpret( CCFBot *sender, CCFBot *receiver ) const = 0;	///< cause the given bot to act on this meme
};
...
public:
	...
	virtual void Interpret( CCFBot *sender, CCFBot *receiver ) const;		///< cause the given bot to act on this meme
...
{
public:
	BotBombStatusMeme( CFGameState::BombState state, const Vector &pos )
	{
		m_state = state;
		m_pos = pos;
	}
...
public:
	...
	CCFBot *GetOwner( void ) const							{ return m_me; }
...
private:
	...
	CCFBot *m_me;														///< the bot this chatter is for
